...
class MoveCommandSpec extends FlatSpec with Matchers {
  ...
  val moveCommandGlobal = MoveCommand(fieldGlobal, PlayerNameEnum.GOLD, new Position(1, 2), new Position(1, 3))
  ...
  "doCommand" should "move the tile to the given position" in {
    ...
    moveCommandGlobal.doCommand() should be(MessageText.doMove(new Position(1, 2), new Position(1, 3)))
  ...
  "undoCommand" should "move the tile back to the given position" in {
    ...
    moveCommandGlobal.undoCommand() should be(MessageText.undoMove(new Position(1, 2), new Position(1, 3)))
